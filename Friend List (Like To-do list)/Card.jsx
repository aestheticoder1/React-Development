...
function Card({removeItem,item,index}) {
  return(
    <div className='w-64 h-72 bg-white p-2 flex flex-col justify-center'>
      {/* IMAGE CONTAINER  */}
      <div className='w-[36%] h-[32%] bg-gray-500 overflow-hidden rounded-full mx-auto'>
        <img className='w-full h-full object-cover' src={item.your_image} alt="" />  
      </div>

      {/* CONTENT CONTAINER  */}
      <div className='flex flex-col gap-2 overflow-hidden'>
        <h2 className='text-black text-lg font-bold text-center mt-1'>{item.your_name}</h2>
        <h3 className='text-md opacity-65 font-semibold text-center leading-3'>{item.your_email}</h3>
        <p className='text-center text-xs mt-3 overflow-hidden px-2'>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Maxime odio quas eveniet et. Omnist. Amet, modi!</p>
      </div>


      <button onClick={()=>removeItem(index)} className='bg-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold ml-[50%] -translate-x-[50%] mt-2.5'>REMOVE</button>
